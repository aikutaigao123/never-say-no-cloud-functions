...
// 邮件验证云函数路由
app.post('/sendVerificationEmail', async function(req, res) {
  const { email, token } = req.body;
  
  if (!email || !token) {
    return res.status(400).json({ 
      success: false, 
      error: '缺少必要参数：email 和 token' 
    });
  }
...
// 检查邮箱验证状态云函数路由
app.post('/checkEmailVerificationStatus', async function(req, res) {
